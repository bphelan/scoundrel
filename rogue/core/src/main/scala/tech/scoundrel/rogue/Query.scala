...
import tech.scoundrel.rogue
import tech.scoundrel.rogue.MongoHelpers.{ AndCondition, FieldOrderTerm, MongoBuilder, MongoModify, MongoOrder, MongoSelect, NaturalOrderTerm, ScoreOrderTerm, SearchCondition }
import tech.scoundrel.rogue.index.MongoIndex
...
case class Query[M, R, +State](
   ...
   */
  def or[S2](subqueries: (Query[M, R, rogue.Ordered with Selected with Limited with Skipped with HasNoOrClause] => Query[M, R, _])*)(implicit ev: AddOrClause[State, S2]): Query[M, R, S2] = {
    val queryBuilder =
      this.copy[M, R, rogue.Ordered with Selected with Limited with Skipped with HasNoOrClause](
        lim = None,
        sk = None,
        maxScan = None,
        comment = None,
  ...
  def andAsc(field: M => AbstractQueryField[_, _, _, M])(implicit ev: State <:< rogue.Ordered): Query[M, R, State] =
    this.copy(order = Some(MongoOrder(FieldOrderTerm(field(meta).field.name, true) :: order.get.terms)))

  def andDesc(field: M => AbstractQueryField[_, _, _, M])(implicit ev: State <:< rogue.Ordered): Query[M, R, State] =
